...
@Version: 1.0
@Autor: Vicro
@Date: 2020-07-24 21:30:31
@LastEditors: Vicro
@LastEditTime: 2020-07-25 22:57:46
'''
import torch
...
parser.add_argument('--outf', default='./model',
                    help='folder to output images and checkpoints')  # 输出结果保存路径
parser.add_argument('--pre_model', default=False,
                    help='use pre_model')  # 恢复训练时模型的路径
...
data_image = {x: datasets.ImageFolder(root=os.path.join(
    path, x), transform=transform) for x in ["train", "val"]}
